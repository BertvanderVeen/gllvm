...
ordiplot.gllvm <- function(object, biplot = FALSE, ind.spp = NULL, alpha = 0.5, main = NULL, which.lvs = c(1, 2), predict.region = FALSE, level =0.95,
  ...
  if(length(cex.spp)==1){
    cex.spp <- rep(cex.spp,p)
  }else if(length(cex.spp)!=p){
    stop("cex.spp needs to be of length p or 1.")
  }
    ...
    if (biplot) {
      ...
      if (!jitter){
        ...
        text(
          ...
          col = spp.colors[largest.lnorms][1:ind.spp][apply(idx[largest.lnorms[1:ind.spp],which.lvs,drop=F],1,function(x)all(x))], 
          cex = cex.spp[largest.lnorms][1:ind.spp][apply(idx[largest.lnorms[1:ind.spp],which.lvs,drop=F],1,function(x)all(x))])
      }
      if (jitter){
        if (symbols) {
          points(choose.lvs[, which.lvs[1]] + runif(Nlv,-a,a), (choose.lvs[, which.lvs[2]] + runif(Nlv,-a,a)), col =
      ...
      if(spp.arrows){
        ...
        if(nrow(scores_to_plot)>0){
          ...
          if(nrow(ends)>0){
            arrows(origin[1],origin[2],ends[,1]+origin[1],ends[,2]+origin[2],col=spp.colors[largest.lnorms][1:ind.spp][!apply(idx[largest.lnorms[1:ind.spp],which.lvs,drop=F],1,function(x)all(x))][idx2], cex = cex.spp[largest.lnorms][1:ind.spp][!apply(idx[largest.lnorms[1:ind.spp],which.lvs,drop=F],1,function(x)all(x))][idx2], length = 0.1, lty=spp.arrows.lty)
            text(x=ends[,1]*(1+lab.dist)+origin[1],y=ends[,2]*(1+lab.dist)+origin[2],labels = row.names(scores_to_plot),col=spp.colors[largest.lnorms][1:ind.spp][!apply(idx[largest.lnorms[1:ind.spp],which.lvs,drop=F],1,function(x)all(x))], cex = cex.spp[largest.lnorms][1:ind.spp][!apply(idx[largest.lnorms[1:ind.spp],which.lvs,drop=F],1,function(x)all(x))])
          }
        }
      }
    }
...
#'@export ordiplot.gllvmHO
#'@method ordiplot gllvmHO
ordiplot.gllvmHO <- function(object, biplot = FALSE, ind.spp = NULL, alpha = 0.5,
                              main = NULL, which.lvs = c(1, 2),
                              predict.region = FALSE, level = 0.95,
                              jitter = FALSE, jitter.amount = 0.2,
                              s.colors = 1, s.cex = 1.2, symbols = FALSE,
                              cex.spp = 0.7, spp.colors = "blue",
                              lwd.ellips = 0.5, col.ellips = 4, lty.ellips = 1,
                              col.spp.ellips = "blue", lty.spp.ellips = 2,
                              rotate = TRUE, ...) {
  n <- nrow(object$y)
  p <- ncol(object$y)
  d <- object$num.lv

  if (d < 1) stop("No latent variables to plot.")

  sigma   <- object$params$sigma.lv
  theta   <- sweep(object$params$theta, 2, sigma, `*`)  # p x d species scores
  if (is.null(rownames(theta)))
    rownames(theta) <- if (!is.null(colnames(object$y))) colnames(object$y) else paste0("sp", seq_len(p))

  lv <- object$lvs   # n x d site scores
  ...
  # Post-hoc SVD rotation for interpretability
  rot <- if (rotate && d > 1) svd(lv)$v else diag(d)
  lv_rot <- lv    %*% rot
  th_rot <- theta %*% rot

  # Biplot scaling (same alpha convention as ordiplot.gllvm)
  bothnorms <- sapply(which.lvs, function(k)
    sqrt(sum(lv_rot[, k]^2)) * sqrt(sum(th_rot[seq_len(ind.spp), k]^2)))
  col_norms_lv <- sqrt(colSums(lv_rot^2))
  col_norms_th <- sqrt(colSums(th_rot^2))
  ...
  # Site prediction ellipses (VA variances; no CMSEP for HO)
  if (!isFALSE(predict.region) && (isTRUE(predict.region) || predict.region == "sites")) {
    rad <- sqrt(qchisq(level, df = 2))
    sscale <- bothnorms^alpha / col_norms_lv
    B <- (diag(sscale) %*% rot)[c(xl, yl), , drop = FALSE]
    for (i in seq_len(n)) {
  ...
  if (symbols) {
    ...
    labs <- if (!is.null(rownames(lv))) rownames(lv) else as.character(seq_len(n))
    text(lv_sc[, c(xl, yl)], labels = labs, cex = s.cex, col = s.colors)
  }

  # Species scores (biplot)
  if (biplot) {
    if (!isFALSE(predict.region) && predict.region == "species") {
      # Uncertainty ellipses for random species loadings
      rad_sp  <- sqrt(qchisq(level, df = 2))
      tscale  <- bothnorms^(1 - alpha) / col_norms_th
      Bsp     <- (diag(tscale) %*% rot)[c(xl, yl), , drop = FALSE]
      # Variance of theta_j = sigma * a_j: Var(sigma_k * a_jk) = sigma_k^2 * A_j(k,k)
    ...
    text(th_sc[seq_len(ind.spp), c(xl, yl)],
         labels = rownames(theta)[seq_len(ind.spp)],
         cex = cex.spp, col = spp.colors[seq_len(ind.spp)])
  }

  invisible(list(sites = lv_sc[, c(xl, yl)], species = th_sc[, c(xl, yl)]))
}
