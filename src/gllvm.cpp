  ...
  if((method<1) | (method>1)){
    ...
    if((random(2)>0) && ((num_RR+num_lv_c)>0)){
      ...
      if(sbl3 == Klv){//randomB="P" & randomB=="single"
        ...
        for(int q=0; q<(num_lv_c+num_RR); q++){
          nll.array() -= ((((vector <Type> (AB_lv.col(q).matrix().diagonal())).log()).sum() - 0.5*((Sigmab_lv.col(q).matrix()).inverse()*(AB_lv.col(q).matrix()*AB_lv.col(q).matrix().transpose())).trace()-0.5*(b_lv.col(q).transpose()*((Sigmab_lv.col(q).matrix()).inverse()*b_lv.col(q))).sum()))/(n*p);// log(det(A_bj))-sum(trace(S^(-1)A_bj))*0.5 + a_bj*(S^(-1))*a_bj
          nll.array() -= 0.5*(Klv-((vector <Type> (Sigmab_lv.col(q).matrix().diagonal())).log()).sum())/(n*p);
        }
      }
    ...
    if(random(2)>0){
      ...
      if(sbl3==Klv){
        ...
        for (int q=0; q<(num_lv_c+num_RR); q++) {
          nll.array() += MVNORM(Sigmab_lv.col(q).matrix())(b_lv.col(q))/(n*p);
        }
      }
